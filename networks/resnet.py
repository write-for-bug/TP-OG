...
class OurResnet(nn.Module):
    def __init__(self):
        ...
        # self.classifier = nn.Linear(self.dim_in, 2)

    def forward(self, x):
        feat = self.encoder(x).squeeze()
        unnormalized_feat = self.head(feat)
        normalized_feat = F.normalize(unnormalized_feat, dim=1)
    ...
    def intermediate_forward_head(self, x):
        feat = self.encoder(x).squeeze()
        unnormalized_feat = self.head(feat)
        return unnormalized_feat
